...
const CreateCategory = () => {
  ...
  const handleSubmit = async (e)=>{
    e.preventDefault()
    try {
      const{data}=await axios.post('http://localhost:3030/api/v1/category/create-category',{name,})
      if (data?.success) {
  ...
  const handleUpdate =async(e)=>{
    ...
    try {
      const {data} = await axios.put(
        `http://localhost:3030/api/v1/category/update-category/${selected._id}`,{name:upadtedName}
      );
      if (data.success) {
