...
const UpdateProduct = () => {
  ...
  const getSingleProduct = async () => {
    try {
      const { data } = await axios.get(`http://localhost:3030/api/v1/product/get-product/${params.slug}`);
      if (data?.success) {
  ...
  const getAllCategory = async () => {
    try {
      const { data } = await axios.get("http://localhost:3030/api/v1/category/get-category");
      if (data?.success) {
  ...
  const handleDelete =async()=>{
    try {
      // eslint-disable-next-line
      const { data }=await axios.delete(`http://localhost:3030/api/v1/product/delete-product/${id}`)
